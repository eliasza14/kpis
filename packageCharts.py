...
def pctChangeV2(categories, values, line_labels, yaxis_title, legend_bar):
    ...
    # Add labels inside the bars with dynamic y-positioning
    y_offset = max(values) * 0.05  # Adjust this value to change the vertical offset
    for i in range(len(categories)):
        label = line_labels[i]
        if not pd.isna(label):
            fig.add_annotation(x=categories[i], y=values[i] + y_offset, text=f"<b>{label} %</b>", showarrow=False,
                               font=dict(color='#F0894F', size=15))

    # Add values at the top of each bar
    for i in range(len(values)):
        fig.add_annotation(x=categories[i], y=values[i], text=f"<b>{str(round((values[i]), 1))}</b>", showarrow=False,
                           font=dict(color='white', size=15),  textangle=0, xanchor='center', yanchor='bottom')
...
#                            font=dict(color='white', size=15),  textangle=270, xanchor='center', yanchor='middle')
